...
function poly(points,m,y=0){const s=new T.Shape();points.forEach(([x,z],i)=>i?s.lineTo(x,-z):s.moveTo(x,-z));s.closePath();const o=add(new T.ShapeGeometry(s),m,0,y,0);o.rotation.x=-Math.PI/2;o.castShadow=false;return o}
...
// Traced from the user-supplied Yandex satellite screenshot, 10 September 2026.
...
roads.forEach(([width,points])=>strip(path(points),width*U,asphalt,.04));
function curve(p,y=.10){return new T.CatmullRomCurve3(p.map(([x,z])=>new T.Vector3(x,y,z)),false,'centripetal')}
function track(points){const c=curve(points),n=Math.ceil(c.getLength()/.65);for(let k=0;k<n;k++){const p=c.getPointAt(k/n),v=c.getTangentAt(k/n),b=box(p.x,p.z,.72,.065,.07,sleeper);b.rotation.y=Math.atan2(v.x,v.z)}for(let side of [-.19,.19]){const pp=[];for(let i=0;i<=120;i++){const p=c.getPointAt(i/120),v=c.getTangentAt(i/120);pp.push(p.add(new T.Vector3(v.z,0,-v.x).multiplyScalar(side)))}add(new T.TubeGeometry(new T.CatmullRomCurve3(pp),160,.018,5,false),rail,0,0,0)}return c}
const tracks=[];for(let j=0;j<13;j++)tracks.push(track(path([[85,401+j*3],[260,417+j*2.6],[465,397+j*2.6],[667,365+j*2.7],[803,332+j*2.8],[915,306+j*1.4]])));
const depot=track(path([[860,353],[902,390],[955,423],[1020,460]]));
track(path([[684,389],[719,376],[821,370],[915,418]]));
...
function storage(x,z,r,h,cat,f){
 ...
 const steel=surfaces.steel.clone(),ribSteel=mat('#9ea6a8',{metalness:.7,roughness:.34});

 cylinder(0,0,r+.32,.18,paving,.08,g);cylinder(0,0,r+.13,.39,steel,.22,g);
 const shell=cylinder(0,0,r,h,steel,.55,g);
 const body=cylinder(0,0,r*.94,1,categories[cat],.55,g);
 ...
 for(let k=1;k<=7;k++)ring(0,.55+h*k/8,0,r+.014,.015,ribSteel,g);
...
const placements=tankPixels.map(([px,py,r],i)=>[...plan(px,py),r*MPP[0]*U,Math.min(4.5,Math.max(2.6,r*MPP[0]*U*.95)),i%4,[.2,.65,0,.85,1,.4][i%6]]);
placements.forEach(a=>storage(...a));
const site=buildInfrastructure({fixed,dynamic,box,poly,line,cylinder,ring,mat,asphalt,paving,silver,dark,windows,white,tanks});
...
const capGeo=new T.CapsuleGeometry(.36,1.60,4,16);
function wagon(cat,parent=fixed){const g=new T.Group();parent.add(g);g.name='rail-tank-car';g.userData.nominalMetres={length:12,width:3.12};
box(0,0,.74,2.8,.10,dark,.27,g);const tank=add(capGeo,white,0,.70,0,g);tank.rotation.x=Math.PI/2;
for(const z of [-.72,.72]){const rr=ring(0,.70,z,.365,.026,silver,g);rr.rotation.x=0;}
for(const x of [-.20,.20])for(const z of [-.90,-.63,.63,.90]){const w=add(new T.CylinderGeometry(.115,.115,.06,12),dark,x,.115,z,g);w.rotation.z=Math.PI/2;}
cylinder(0,0,.09,.05,silver,1.04,g);box(0,1.45,.12,.10,.055,dark,.29,g);box(0,-1.45,.12,.10,.055,dark,.29,g);return g;}
function onCurve(g,c,u){const p=c.getPointAt(u),d=c.getTangentAt(u);g.position.copy(p);g.rotation.y=Math.atan2(d.x,d.z)}
for(const [j,cat,start,count] of [[1,0,.07,17],[4,1,.14,20],[7,3,.25,19],[10,2,.47,15]]){const c=tracks[j],step=3.12/c.getLength();for(let n=0;n<count;n++)onCurve(wagon(cat),c,start+n*step)}
for(let n=0;n<12;n++)onCurve(wagon(Math.floor(n/4)%4),depot,.025+n*3.12/depot.getLength());
const movingWagons=[];
for(let n=0;n<6;n++){const g=wagon((n+1)%4,dynamic);movingWagons.push({g,offset:n*3.12/tracks[12].getLength()});}
let trainU=.78,trainDirection=1;
...
function truck(cat,parent=fixed){
 const g=new T.Group();parent.add(g);g.scale.setScalar(.37);g.name="road-tanker";g.userData.nominalMetres={length:10.7,width:2.6};
 box(0,-.2,1.1,6.9,.24,dark,.63,g);
 const tank=add(new T.CapsuleGeometry(.78,3.7,8,24),white,0,1.76,-1.19,g);tank.rotation.x=Math.PI/2;
...
const containerMats=['#b9c4b3','#d7ba8e','#d6c9ad','#a9bcc7'].map(c=>mat(c));for(let row=0;row<10;row++)for(let col=0;col<3;col++){const [x,z]=plan(515+col*23+row*2.6,148+row*7.4);const c=box(x,z,3.048,.610,.648,containerMats[(row+col)%4]);c.rotation.y=-.48;}
function vessel(x,z,L,W,cat,rot){const g=new T.Group();g.position.set(x,-.4,z);g.rotation.y=rot;fixed.add(g);const s=new T.Shape();s.moveTo(-W*.42,-L*.48);s.lineTo(W*.42,-L*.48);s.lineTo(W*.5,L*.25);s.quadraticCurveTo(W*.43,L*.43,0,L*.52);s.quadraticCurveTo(-W*.43,L*.43,-W*.5,L*.25);s.closePath();
 const hull=add(new T.ExtrudeGeometry(s,{depth:2.3,bevelEnabled:true,bevelSize:.25,bevelThickness:.2,bevelSegments:2,steps:1}),mat('#354652',{roughness:.6,metalness:.3}),0,.3,0,g);hull.rotation.x=-Math.PI/2;const deck=add(new T.ShapeGeometry(s),silver,0,2.72,0,g);deck.rotation.x=-Math.PI/2;
...
mappedVessel(520,220,40,7.4,3,-.48);mappedVessel(386,285,26,5.4,1,1.28);mappedVessel(265,260,30,5.8,2,1.38);
...
for(const [px,py,angle] of [[546,218,-.4],[492,145,-.4],[382,306,1.3],[285,285,1.4]]){const [x,z]=plan(px,py);crane(x,z,angle);}
...
function equipment(node,pos) {
 const g=new T.Group();g.position.copy(pos);equipmentRoot.add(g);g.scale.setScalar(.28);
...
function bind(nodes) {
 ...
 for(const node of nodes.values()){const model=site.buildingModels.get(node.buildingId||node.id);if(model){records.set(node.id,{group:model});tagged(model,node.id);}}
 for(const node of nodes.values()){
  if(records.has(node.id)||['terminal','zone','tank'].includes(node.type))continue;
  const parent=records.get(node.parent),siblings=catalog.get(node.parent).children,index=siblings.indexOf(node.id);
  let position;
  if(parent){const base=parent.group.position;const radius=parent.tank?parent.tank.r+.6:.5;const a=-Math.PI*.2+index*.85;position=new T.Vector3(base.x+Math.cos(a)*radius,0,base.z+Math.sin(a)*radius);}
  else {const coords=node.parent==='rail'?plan(681,386):node.parent==='port'?plan(682,181):plan(640,477);position=new T.Vector3(coords[0]+index*6,0,coords[1]);}
 ...
 for(const zone of [...catalog.values()].filter(n=>n.type==='zone')){
  ...
  plane.rotation.x=-Math.PI/2;plane.position.set(center.x,.20,center.z);plane.userData.entityId=zone.id;scene.add(plane);zoneMeshes.push(plane);
...
function selectEntity(id,focus=true){
 ...
 for(const [key,rec] of records){const n=catalog.get(key);
  if(n.type==='zone'){rec.plane.material.opacity=isWithin(id,key)?.07:0;rec.outline.material.opacity=isWithin(id,key)?.85:0;continue;}
  if(n.type==='tank'){const cut=activeTank?.id===key;rec.tank.roof.visible=!cut;const m=rec.tank.shell.material;if(m.transparent!==cut){m.transparent=cut;m.depthWrite=!cut;m.needsUpdate=true;}m.opacity=cut?0.18:1;continue;}
