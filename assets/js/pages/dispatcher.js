(function () {
  ...
  const state = { mode: "fallback", zoom: 17, active: new Set(["zones","pipelines","equipment"]), overlays: {}, map: null };
  ...
  function buildGoogleOverlays() {
    data.zones.forEach(zone => {
      ...
      polygon.addListener("click", () => openDrawer({name:zone.name,type:"РћРїРµСЂР°С†РёРѕРЅРЅР°СЏ Р·РѕРЅР°",status:"Р’ СЌРєСЃРїР»СѓР°С‚Р°С†РёРё",tone:"green",load:68,wear:31,risk:18,next:"РџРѕ РіСЂР°С„РёРєСѓ",cost:"$34 000",recommendation:"РћС‚РєР»РѕРЅРµРЅРёР№ РїРѕ РїСЂРѕРїСѓСЃРєРЅРѕР№ СЃРїРѕСЃРѕР±РЅРѕСЃС‚Рё РЅРµС‚.",id:zone.id}));
    ...
    data.tanks.forEach(tank => {
      ...
      push("tanks", circle, {minZoom:tank.major?18:19});
    ...
    data.maintenance.forEach(item => push("maintenance", new google.maps.Marker({position:item.position,title:item.name,label:{text:"РўРћ",color:"#fff",fontSize:"8px",fontWeight:"700"},icon:{path:google.maps.SymbolPath.CIRCLE,scale:10,fillColor:"#e99919",fillOpacity:1,strokeColor:"#fff",strokeWeight:2}}), {minZoom:19}));
    ...
    data.procurement.forEach(item => push("procurement", new google.maps.Marker({position:item.position,title:item.name,label:{text:"Р—РРџ",color:"#fff",fontSize:"8px",fontWeight:"700"},icon:{path:google.maps.SymbolPath.CIRCLE,scale:12,fillColor:"#705ac8",fillOpacity:1,strokeColor:"#fff",strokeWeight:2}}), {minZoom:17}));
  }
  ...
  function buildFallback() {
    ...
    const maintenance = data.maintenance.map(item => {const p=project(item.position);return `<i data-fallback-layer="maintenance" class="fallback-mini maintenance" style="left:${p.left}%;top:${p.top}%">РўРћ</i>`;}).join("");
    const procurement = data.procurement.map(item => {const p=project(item.position);return `<i data-fallback-layer="procurement" class="fallback-mini procurement" style="left:${p.left}%;top:${p.top}%">Р—РРџ</i>`;}).join("");
  ...
  function updateVisibility() {
    ...
    document.querySelectorAll("[data-fallback-layer]").forEach(node => {
      const minZoom = Number(node.dataset.minZoom || 0);
      node.hidden = !state.active.has(node.dataset.fallbackLayer) || state.zoom < minZoom;
    });
    byId("zoom-level").textContent = `LOD ${state.zoom}`;
    byId("layer-status").textContent = `РђРєС‚РёРІРЅРѕ: ${data.layers.filter(layer => state.active.has(layer[0])).map(layer => layer[1]).join(" В· ")}`;
  }

  function openTank(tank) {
    openDrawer({id:tank.id,name:`Р РµР·РµСЂРІСѓР°СЂ ${tank.id}`,type:"Р РµР·РµСЂРІСѓР°СЂРЅС‹Р№ РїР°СЂРє",status:tank.critical?"Р’С‹СЃРѕРєРёР№ СЂРёСЃРє":"Р’ СЌРєСЃРїР»СѓР°С‚Р°С†РёРё",tone:tank.critical?"red":"green",load:tank.load,wear:tank.wear,risk:tank.critical?76:18,next:tank.next,cost:"$12 600",recommendation:tank.critical?"РћРіСЂР°РЅРёС‡РёС‚СЊ Р·Р°РіСЂСѓР·РєСѓ РґРѕ 72% Рё РІС‹РїРѕР»РЅРёС‚СЊ РєРѕРЅС‚СЂРѕР»СЊ С‚РѕР»С‰РёРЅС‹ СЃС‚РµРЅРєРё.":"РџСЂРѕРґРѕР»Р¶РёС‚СЊ СЌРєСЃРїР»СѓР°С‚Р°С†РёСЋ РїРѕ С‚РµРєСѓС‰РµРјСѓ РіСЂР°С„РёРєСѓ."});
  }
  function openDrawer(item) {
    if (!item) return;
    byId("drawer-title").textContent = item.name;
    byId("drawer-subtitle").textContent = item.type;
    byId("drawer-body").innerHTML = `<span class="badge ${item.tone || "green"}">${ui.escape(item.status)}</span><div class="drawer-metrics">${[["Р—Р°РіСЂСѓР·РєР°",`${item.load}%`],["РР·РЅРѕСЃ",`${item.wear || 0}%`],["Р РёСЃРє РїСЂРѕСЃС‚РѕСЏ",`${item.risk}%`],["РЎР»РµРґСѓСЋС‰РµРµ РўРћ",item.next],["РЎС‚РѕРёРјРѕСЃС‚СЊ СЂРµРјРѕРЅС‚РѕРІ",item.cost],["РўРµР»РµРјРµС‚СЂРёСЏ","РћРЅР»Р°Р№РЅ"]].map(([label,value]) => `<div><span>${label}</span><strong>${value}</strong></div>`).join("")}</div><div class="drawer-recommend"><strong>Р РµРєРѕРјРµРЅРґР°С†РёСЏ СЃРёСЃС‚РµРјС‹</strong><p>${ui.escape(item.recommendation)}</p></div><nav class="drawer-links"><a href="/equipment-detail?id=${encodeURIComponent(item.id)}">РџР°СЃРїРѕСЂС‚ РѕР±РѕСЂСѓРґРѕРІР°РЅРёСЏ <b>в†’</b></a><a href="/analytics">РџСЂРµРґРёРєС‚РёРІРЅР°СЏ Р°РЅР°Р»РёС‚РёРєР° <b>в†’</b></a><a href="/toir">Р Р°Р±РѕС‚С‹ РўРћРёР  <b>в†’</b></a><a href="/procurement">Р—Р°РєСѓРїРєРё / Р—РРџ <b>в†’</b></a></nav>`;
  ...
  function bind() {
    ...
    byId("show-risks").addEventListener("click", () => {state.active.add("risks");updateVisibility();ui.toast("Р РёСЃРєРё РІРєР»СЋС‡РµРЅС‹","РќР° РєР°СЂС‚Рµ РїРѕРєР°Р·Р°РЅС‹ РѕР±СЉРµРєС‚С‹ РїРѕРІС‹С€РµРЅРЅРѕРіРѕ РІРЅРёРјР°РЅРёСЏ");});
    ...
    byId("map-search").addEventListener("keydown", event => {
      ...
      else ui.toast("РћР±СЉРµРєС‚ РЅРµ РЅР°Р№РґРµРЅ","РџСЂРѕРІРµСЂСЊС‚Рµ РѕР±РѕР·РЅР°С‡РµРЅРёРµ РёР»Рё РІРЅСѓС‚СЂРµРЅРЅРёР№ РЅРѕРјРµСЂ");
    });
  }
  ...
  async function init() {
    ...
    try {
      ...
      byId("google-map").hidden=false; byId("fallback-map").hidden=true; byId("map-mode").textContent="Google Maps В· Live";
      updateVisibility();
    } catch (_) {
      byId("map-mode").textContent="Satellite В· Demo";
    }
  }
